...
int main(void)
{
    ...
    client_left_semafor = sem_open("client_left_semafor", 0);
...
void * print_game(void * global_data)
{
    while (true)
    {
        ...
        if (baza_flaga)
        {
            mvprintw(1, 54, "Campsite X/Y: %02d/%02d", baza_x, baza_y);
        }
...
void * quit(void * global_data)
{
    ...
    {
        int przycisk = getch();
        if (przycisk == 'q' || przycisk == 'Q')
        {
            ((struct client_dane *)global_data)->x = -4;
            ((struct client_dane *)global_data)->y = -4;
            sem_post(client_left_semafor);
            endwin();
            exit(0);
        }
